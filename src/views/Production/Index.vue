<template>
  <section class="pb-20">
    ...
    >
      <ul class="basicTab">
        <li
          v-if="$can('read', 'production-coming-raw')"
        ...
        <li
          v-if="$can('read', 'production-coming-ready')"
        ...
        <li
          v-if="$can('read', 'production-expense')"
        ...
        <li
          v-if="$can('read', 'production-warehouse')"
        ...
        <li
          v-if="$can('read', 'production-ready-warehouse')"
      ...
      <div>
        <button
          v-if="$can('read', 'production-moving')"
          @click="toggleSendFoamModal"
          class="mainBackgroundColorBlue text-white basicBtn mr-4"
        >
        ...
        <button
          v-if="$can('read', 'production-produce')"
          @click="toggleModal"
    ...
    <div class="a-table border-b-[1.5px] borderBox dark:border-[#1a283a] min-h-[500px]">
      <div class="table-1" v-if="params.currentTable === 'table-1' && $can('read', 'production-coming-raw')">
        <div class="wrapper">
          ...
          <div class="a-table-body" v-if="!isLoading && !isError">
            ...
            >
              ...
              <li class="uk-width-expand">
                <h3 class="a-table-text  main-color-black" :class="$isDark ? 'dark' : ''">{{ item.product?.code }}</h3>
              </li>
              <li class="uk-width-expand">
              ...
              <li class="w-[100px] relative">
                <v-confirm v-if="confirmToDeleteProduct == item.id" classes="right-[100px] left-auto top-[15px] -translate-y-1/2" :question="$t('are_realy_want_to_delete_this')" @cancel="() => {confirmToDeleteProduct = null}" @confirm="console.log('aaa')" />
                <div
      ...
      <div class="table-2" v-if="params.currentTable === 'table-2' && $can('read', 'production-coming-ready')">
        <div class="wrapper">
          <div class="a-table-header">
            <ul class="uk-grid-collapse a-table-list" uk-grid>
              <li class="uk-width-expand">
          ...
          <div class="a-table-body" v-if="!isLoading && !isError">
            ...
            >
              ...
              <li class="uk-width-expand">
                <h3 class="a-table-text main-color-black">{{item.product && item.product[`name_${locale}`] ? item.product[`name_${locale}`] : ''}}</h3>
              </li>
              <li class="uk-width-expand">
              ...
              <li class="uk-width-expand">
                <div class="inline-block relative group">
                  <h3 class="a-table-text mainColorText blue-text">{{$t('show')}}</h3>
                  <div
                  ...
                  >
                    <div class="relative w-full">
                      <div v-for="(item2, index2) in item.order_extra_products" :key="index2">
                        <span class="text-[12px] text-[#888A8F]">
                          {{item2.product && item2.product[`name_${locale}`] ? item2.product[`name_${locale}`] : ''}}
                        </span>
      ...
      <div class="table-3" v-if="params.currentTable === 'table-3' && $can('read', 'production-expense')">
        <div class="wrapper">
          <div class="a-table-header">
            <ul class="uk-grid-collapse a-table-list" uk-grid>
              <li class="uk-width-expand">
          ...
          <div class="a-table-body" v-if="!isError && !isLoading">
            <ul
            ...
            >
              ...
              <li class="uk-width-expand">
                <h3 class="a-table-text main-color-black">{{ item.product?.name_ru }}</h3>
              </li>
              <li class="uk-width-expand">
                <h3 class="a-table-text warn-color" :class="$isDark ? 'dark' : ''">{{ formatPrice(item.amount) }} {{item.unit}}</h3>
              </li>
              ...
              <li class="uk-width-expand">
                <h3 class="a-table-text warn-color">
                  {{ item && item.order && item.order.product && item.order.product[`name_${locale}`] ? item.order.product[`name_${locale}`] : '' }} 
                  ({{ item?.order?.product?.code }}) - {{ item?.order?.amount }} {{ item?.order?.unit }}</h3>
              </li>
              <li class="w-[100px] relative">
                <div
                  @click="togglePopup(item.id)"
      ...
      <div class="table-4" v-if="params.currentTable === 'table-4' && $can('read', 'production-warehouse')">
        <div class="wrapper">
          <div
                ...
                <div class="uk-grid-collapse uk-child-width-1-2" uk-grid>
                  <div>
                    <div class="border-r-[1.5px] borderBox dark:border-[#1a283a]  pr-10">
                      ...
                      <div class="a-table-body" v-if="!isLoading">
                        <ul
                          v-for="(item, index) in storeListLeft"
                          class="uk-grid-collapse a-table-list border-t-[1.5px] borderBox dark:border-[#1a283a] "
                          uk-grid :key="index"
                        >
                          <li class="uk-width-expand">
                  ...
                  <div>
                    <div class="pl-10">
                      ...
                      <div class="a-card-body" v-if="!isLoading">
                        <ul
                          v-for="(item, index) in storeListRight"
                          class="uk-grid-collapse a-table-list border-t-[1.5px] borderBox dark:border-[#1a283a] "
                          uk-grid :key="index"
                        >
                          <li class="uk-width-expand">
                      ...
                      <Skeleton v-if="isLoading && $route.query.table == 'table-4'" classes="w-full min-h-[500px]  h-full" />
                      <null-data v-else-if="isErrorLeft || !storeListRight.length" width='w-[120px]' classes='h-full min-h-[500px]  h-full' />
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div class="table-5" v-if="params.currentTable === 'table-5' && $can('read', 'production-ready-warehouse')">
        <div class="wrapper">
          <div
            class="uk-grid-medium uk-grid-match uk-child-width-expand"
            uk-grid
                ...
                <div class="uk-grid-collapse uk-child-width-1-2" uk-grid>
                  <div>
                    <div class="border-r-[1.5px] borderBox dark:border-[#1a283a]  pr-10">
                      ...
                      <Skeleton v-if="isLoading && $route.query.table == 'table-5'" classes="w-full min-h-[500px]  h-full" />
                      <null-data v-else-if="isErrorLeft || !storeListLeft.length" width='w-[120px]' classes='h-full min-h-[500px]  h-full' />
                    </div>
                  </div>
                  <div>
    ...
    <div class="flex justify-end mt-10 wrapper" v-if="!isLoading && !isError">
        <v-pagination
          v-if="params.currentTable == 'table-1'"
          v-model="params.pagination.page"
          :pages="params.pagination.total_1"
          :range-size="1"
        ...
        />
        <v-pagination
          v-else-if="params.currentTable == 'table-2'"
          v-model="params.pagination.page"
          :pages="params.pagination.total_2"
          :range-size="1"
          @update:modelValue="updatePage"
          :hideFirstButton="true"
          hideLastButton="true"
        />
        <v-pagination
          v-else-if="params.currentTable == 'table-3'"
          v-model="params.pagination.page"
          :pages="params.pagination.total_3"
          :range-size="1"
          @update:modelValue="updatePage"
          :hideFirstButton="true"
          hideLastButton="true"
        />
        <v-pagination
          v-else-if="params.currentTable == 'table-4'"
          v-model="params.pagination.page"
          :pages="params.pagination.total_4"
          :range-size="1"
          @update:modelValue="updatePage"
          :hideFirstButton="true"
          hideLastButton="true"
        ...
        <v-pagination
          v-else
          v-model="params.pagination.page"
          :pages="params.pagination.total_5"
          :range-size="1"
          @update:modelValue="updatePage"
          :hideFirstButton="true"
          hideLastButton="true"
    ...
    <productionModal :isModalActive="isModalActive" v-if="$can('read', 'production-produce')"/>
    <sendFoamModal :isModalActive="isSendFoamModalActive" v-if="$can('read', 'production-moving')"/>
  </section>
...
import sendFoamModal from './components/sendFoamModal.vue'
...
export default {
  ...
  setup() {
    ...
    let isSendFoamModalActive = ref(false)
    ...
    const toggleSendFoamModal = () => {
      isSendFoamModalActive.value = !isSendFoamModalActive.value
    }
    ...
    const changeTable = async (e) => {
      router.push(`/production?table=${e}`)
    ...
    const getFactoryIncome = async () => {
      ...
      await factoryIncomeStorage.GET_FACTORY_INCOME(data).then(response => {
        ...
        params.value.totalCount1 = response.data.data.total
        params.value.pagination.total_1 = response.data.data.total > 9 ? Math.ceil(response.data.data.total / 10) : 1
    ...
    const getOrders = async () => {
      ...
      await orderStorage.GET_ORDER(data).then(response => {
        ...
        params.value.totalCount2 = response.data.data.total
        params.value.pagination.total_2 = response.data.data.total > 9 ? Math.ceil(response.data.data.total / 10) : 1
        if (!response.data.data.data.length) {
          isError.value = true
        } else {
          isError.value = false
    ...
    const getOrdersOutcome = async () => {
      ...
      await orderStorage.GET_ORDER_EXTRA_PRODUCT(data).then(response => {
        ...
        params.value.totalCount3 = response.data.data.total
        params.value.pagination.total_3 = response.data.data.total > 9 ? Math.ceil(response.data.data.total / 10) : 1
        if (!response.data.data.data.length) {
          isError.value = true
        } else {
          isError.value = false
    ...
    const getStoreList = async () => {
      ...
      await factoryProductStorage.GET_FACTORY_PRODUCT(data).then(response => {
        ...
        params.value.totalCount4 = response.data.data.total
        params.value.pagination.total_4 = response.data.data.total > 9 ? Math.ceil(response.data.data.total / 10) : 1
      }) 
      .catch(() => {
    ...
    const getStoreReadyProducts = async () => {
      ...
      await factoryProductStorage.GET_FACTORY_PRODUCT(data).then(response => {
        ...
        params.value.totalCount5 = response.data.data.total
        params.value.pagination.total_5 = response.data.data.total > 9 ? Math.ceil(response.data.data.total / 10) : 1
      })
      .catch(() => {
        setTimeout(() => {
          isError.value = true
    ...
    const deleteProductStore = async (id) => {
      ...
      await factoryIncomeStorage.DELETE_RAW_MATERIALS_STORAGE(id).then( async (response) => {
          if (response.data) {
