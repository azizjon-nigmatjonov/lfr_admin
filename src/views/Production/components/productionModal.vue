<template>
   <Modal :isModalActive="isModalActive" @close="toggleModal" width="w-[870px]" @submitModal="onSubmit">
    ...
    <template v-slot:modalBody>
      ...
      <div class="mt-4 grid grid-cols-2 gap-y-5 gap-x-5">
        ...
        <div class="flex flex-col gap-5 max-h-[380px] overflow-y-scroll pr-2">
          <div class="relative flex" v-for="(item, index) in order_raw_foam" :key="index">
            ...
            <input
              v-model="item.title"
              @click="openRawAddModal(item, index)"
              type="text"
              name="text"
              autocomplete="off"
              required
      ...
      <rawMaterialAddModal :isRawAddModalActive="isRawAddModalActive" :rawMaterialItemSelected="rawMaterialItemSelected"/>
    </template>
  </Modal>
</template>
...
export default {
    ...
    setup(props) {
        ...
        let rawMaterialItemSelected = ref()
        ...
        function openRawAddModal(item, index){
          rawAddModalIndex.value = index
          isRawAddModalActive.value = true
          rawMaterialItemSelected = item
        }
          ...
          if (result) {
            await orderStorage.CREATE_ORDER({...form.value, order_extra_products: order_extra_products}).then((response) => {
              ...
              if(response && response.status && response.status == 200){
                ...
                form.value.factory = 'foam-rubber'
                form.value.unit = 'm3'
                form.value.amount = 0
