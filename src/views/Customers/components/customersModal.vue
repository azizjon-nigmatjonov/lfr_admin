...
export default {
    ...
    setup(props) {
      ...
      async function onSubmit(){
        ...
        if (result){
          if (isEdit.value) {
            ...
            await suppliersStorage.UPDATE_SUPPLIER_OR_CUSTOMER({params: data, id: route.query.customer }).then( async response => {
            if (response.status == 200) {
              suppliersStorage.updateCustomer = true
              validate.value.$reset()
              toast.success(t('successfully-edited'))
              await instance.parent.setupState.FETCH_CUSTOMERS_LIST()
              toggleModal()
              isEdit.value = false
      ...
      // ========================= WATCH_EFFECT_SECTION START =========================
      watchEffect(() => {
        if (props.formCustomer) {
          textBtn.value = 'edit'
          isEdit.value = true
          form.value = props.formCustomer
        } else {
          textBtn.value = 'confirm'
          isEdit.value = false
